...
```bash
croc --relay <relay> --pass <relay_pass> send ...
```
...
```bash
croc --relay <relay> --pass <relay_pass>
```
...
- `qim-data` does not auto-install `croc` yet (doctor reports if missing).
- No enterprise auth layer yet; relay security is currently password + network controls.

## Next planned improvements

1. `qim-data install-croc` (or automatic first-run install).
2. OS-native secure storage for relay secret.
3. Additional diagnostics (`qim-data doctor --verbose`).
4. Packaging/release pipeline for Linux/macOS/Windows.
